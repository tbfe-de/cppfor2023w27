...
// Step 10
// TBD: according to the NVI-Idiom you should
// ... "consider to make your public functions non-virtual
//      and your virtual functions non-public"
// The "cost" is that a base class must plan ahead and provide
// "extension points" for derived classes.
// Step in Details:
// - remove the `virtual` from `BaseCounter::is_counting` and
//   `BaseCounter::step`
// - add PRIVATE virtual functions to `BaseCounter`
//  class BaseCounter {
//      ...
//      virtual bool chained_is_counting() const;
//      virtual void chained_needs_step();
//      ...
//  };
// these will be be overridden in the derived class, in the base
// class the first will simply return `false` (always) and the
...
public:
    ...
    {/*empty*/}
    bool is_counting() const override {
        return (value_ > 0)
            || next_.is_counting();
    }
    void step() override;
...
void ChainableDownCounter::step() {
    if (value_ > 0)
        --value_;
    else {
        if (next_.is_counting()) {
            next_.step();
            value_ = reset_-1;
        }
    }
