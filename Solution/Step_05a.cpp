...
bool countClockDown(Clock& clk, int steps = 1) {
    while (steps > 0) {
        if (!clk) // alternatively: if (!clk.operator()) ...
            return false;
...
// The macro `__func__`  used below is standardized in C and is
// available in C++ as it strives for staying compatible with C.
// Its disadvantage is that the function name alone is not
// sufficient in C++ due to overloading. Therefore the macro
// `__PRETTY_FUNCTION__` is available but it is a non-standard
// extension only available in GCC- and LLVM-based compilers (ie.
// g++ and clang++). In Visual Studio C++ a the same functionality
// is available as `__FUNCSIG__`.
